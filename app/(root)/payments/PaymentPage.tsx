...
import { useGetPaymentList } from "@/app/actions/reactQuery";
...
const PaymentPage = () => {
  ...
  // const { data, isLoading } = useGetPaymentList();
  ...
  const { data, isLoading } = useGetPaymentList(filters);
  ...
  id ? `${id.slice(0, 14)}...${id.slice(-6)}` : "N/A";
  ...
  const payments: Payment[] = React.useMemo(() => {
    ...
    return data.map((item: any) => ({
      ...
      // paymentId: shortId(item.reference || item.externalTransactionId || item.id),
      ...
      posted: `Posted: ${new Date(item.createdAt).toLocaleDateString()}`,

      title:
        item.contextType === "subscription"
          ? "Subscription Payment"
          : "Course Payment",

      client: {
        name: item.payer?.email || "Unknown",
        avatar: item.payer?.profilePic || "/assets/images/person3.png",
        role: item.contextType === "subscription" ? "Plan Subscriber" : "Course Student",
      },
          ...
          <div className="flex flex-col gap-4 flex-1">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search by reference, ID, payer..."
                  ...
                  className="w-full pl-5 pr-10 py-3 border border-gray-300 rounded-[12px] focus:outline-none focus:ring-2 focus:ring-[#220084]"
                />
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex-1">
                ...
                <select
                  ...
                  className="w-full border border-gray-300 rounded-[12px] px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#220084]"
                >
              ...
              <div className="flex-1">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-[12px] px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#220084] appearance-none"
                  />

                  {/* calendar icon (non-blocking) */}
              ...
              {isLoading ? (
                ...
                payments.map((payment, index) => (
                  ...
                  >
                    ...
                    <td className="relative group font-mono text-sm text-gray-700">
                      {shortId(payment.paymentId)}

                      <div className="pointer-events-none absolute left-1/2 top-full z-50 mt-2 w-max max-w-xs -translate-x-1/2 scale-95 rounded-md bg-black px-3 py-1.5 text-xs text-white opacity-0 shadow-lg transition-all group-hover:scale-100 group-hover:opacity-100">
                        {payment.paymentId}
                      </div>
                    </td>
